...
use crate::raydium_sdk::{LiquidityStateV4, MarketStateLayoutV3};
...
use {
    ...
    std::{cell::RefCell, str::FromStr, thread, time, time::Duration, vec::Vec},
};
...
impl FarmClient {
    ...
    pub fn get_pool_price_lp(&self, pool_lp: &Pubkey) -> Result<f64, FarmClientError> {
        let account = self.rpc_client.get_account(pool_lp)?;
        let data: Vec<u8> = account.data.clone();
        let market = LiquidityStateV4::try_from_slice(&data).map_err(|e| {
            FarmClientError::ValueError(format!("Failed to unpack MarketStateLayoutV3: {e}"))
        })?;

        let token_a_balance = self
            .rpc_client
            .get_token_account_balance(&market.base_vault)?
            .amount
            .parse::<u64>()
            .unwrap();
        let token_b_balance = self
            .rpc_client
            .get_token_account_balance(&market.quote_vault)?
            .amount
            .parse::<u64>()
            .unwrap();
